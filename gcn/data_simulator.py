...
def sample_data(test_flags,labels,features):
    ...
    tx=np.transpose(sparse.csr_matrix(features[~(bool_ind)]))
    ty=labels[(~bool_ind)]
    bool_ind = np.ones((len(labels)), dtype=bool)
    bool_ind[0:end_train] = False
    y = labels[~(bool_ind)]
    x = np.transpose(sparse.csr_matrix(features[~(bool_ind)]))
...
graph_feat_flags['featureless']=1
